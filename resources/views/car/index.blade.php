<x-app-layout>
    ...
    <div class="p-6 overflow-hidden bg-white rounded-md shadow-md dark:bg-dark-eval-1">
        <div class="mb-5 text-bold text-xl text-black">
            <i class="fas fa-solid fas fa-car"></i>{{ __("My Cars Collection") }}<br>
        ...
        <div class="flex flex-wrap bg-slate-900">
            ...
            <div class="w-64 rounded-lg hover:text-slate-400 hover:shadow-xl hover:bg-transparent shadow-lg p-4 mx-2 my-2">
                <img src="{{ asset('storage/'.$car->image) }}" alt="Image" style="width: 250px; height: 250px;">
                <div class="p-4">
                    <h2>Plate Number: {{ $car->platenumber }}</h2>
                    <h4>Model: {{ $car->model }}</h4>
                    <h4>Brand: {{ $car->brand }}</h4>
                    <h4>Color: {{ $car->color }}</h4>
                    <h4>Date of Purchase: {{ $car->yearbought }}</h4>
                </div>
            </div>
        </a>
            @endforeach
